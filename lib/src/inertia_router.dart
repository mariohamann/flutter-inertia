...
abstract class InertiaRouter {
  ...
  void _addRoute(String method, String pattern, InertiaRouteHandler handler) {
    _routes.add(InertiaRoute(method: method, pattern: pattern, handler: handler));
  }
  ...
  Future<String> handleMessage(String rawJson) async {
    ...
    // If the handler returned a redirect payload, follow it internally
    // with a GET to the redirect target and return that render instead.
    final redirectTarget = _extractRedirectUrl(js);
    if (redirectTarget != null) {
      return await _dispatch('GET', redirectTarget, {});
    }
  ...
  String? _extractRedirectUrl(String js) {
    final match =
        RegExp(r'"redirect"\s*:\s*"([^"]+)"').firstMatch(js);
